<template>
  <section class="page">
    ...
    <el-drawer v-model="drawer" :title="detail ? `运行审计 · ${detail.run_id}` : '运行审计'" size="min(920px, 88%)" @close="resetDetail">
...
async function load() {
  ...
  loadError.value = ''
  try {
    const response = await api.getMootdxXdxrQuality({ limit: 30, startDate: dateRange.value?.[0], endDate: dateRange.value?.[1], status: statusFilter.value || undefined })
    if (requestToken !== loadRequestToken) return
    snapshot.value = response
  } catch (error) {
    if (requestToken !== loadRequestToken) return
    snapshot.value = null
    loadError.value = error instanceof Error ? error.message : '加载 XDXR 质量失败'
...
async function openRun(run: MootdxXdxrRun) {
  ...
  detailStatus.value = ''
  try {
    const response = await api.getMootdxXdxrRunDetail(runId, { limit: 500 })
    if (requestToken !== detailRequestToken || selectedRunId.value !== runId) return
    detail.value = response.item
  } catch (error) {
    if (requestToken !== detailRequestToken || selectedRunId.value !== runId) return
    const message = error instanceof Error ? error.message : '加载运行审计失败'
    ElMessage.error(message)
    drawer.value = false
  } finally {
    if (requestToken === detailRequestToken && selectedRunId.value === runId) detailLoading.value = false
  }
}
async function selectDetailStatus(status: string) {
  ...
  detailLoading.value = true
  try {
    const response = await api.getMootdxXdxrRunDetail(runId, { status: status || undefined, limit: 500 })
    if (requestToken !== detailRequestToken || selectedRunId.value !== runId) return
    detail.value = response.item
  } catch (error) {
    if (requestToken !== detailRequestToken || selectedRunId.value !== runId) return
    ElMessage.error(error instanceof Error ? error.message : '加载审计筛选失败')
  } finally {
    if (requestToken === detailRequestToken && selectedRunId.value === runId) detailLoading.value = false
  }
}
