...
class ConcurrentDetectingClient(FakeClickHouseClient):
    def __init__(self) -> None:
        super().__init__()
        self._active = 0
        self.max_active = 0
...
def test_repository_serializes_concurrent_clickhouse_queries() -> None:
    client = ConcurrentDetectingClient()
    repo = ClickHouseDataOpsRepository(client=client)
    threads = [threading.Thread(target=repo._execute, args=("select 1",)) for _ in range(4)]
...
def test_repository_adds_mootdx_connection_defaults_without_overwriting_schedule() -> None:
    client = FakeClickHouseClient()
    repo = ClickHouseDataOpsRepository(client=client)
    repo.upsert_task_config(
        DataOpsTaskConfig(
            ...
            enabled=True,
            schedule_kind="daily_time",
            schedule_config={"time": "15:40", "limit": 300},
        ),
    ...
    daily = repo._get_config("mootdx_daily_kline_sync")
    assert daily.schedule_config == {
        "time": "15:40",
        "limit": 300,
        "rate_limit": 0.02,
        "timeout": 15,
...
def test_repository_finish_run_can_complete_existing_clickhouse_run() -> None:
    class ExistingRunClient:
        ...
        def execute(self, query: str, params=None):
            ...
            if "select run_id, task_key, status, started_at" in normalized and "where run_id" in normalized:
                return [("run-existing", "quality_snapshot", "running", datetime(2026, 7, 8, 10, 0), None, 0.0, "{}", "")]
    ...
    assert client.inserted[0][1] == "quality_snapshot"
...
def test_repository_failed_heartbeat_interrupts_unfinished_running_run() -> None:
    ...
    repo.write_heartbeat("runner-b", "quality_snapshot", "failed", "interrupted", now=started + timedelta(hours=1))
...
def test_repository_marks_latest_running_task_interrupted() -> None:
    ...
    repo.mark_task_interrupted("quality_snapshot", "runner-b", "上次运行心跳超时，已标记为中断", now=interrupted_at)

    assert client.runs[run_id]["status"] == "failed"
    assert client.runs[run_id]["finished_at"] == interrupted_at
    assert client.heartbeats[("runner-b", "quality_snapshot")]["status"] == "failed"
...
def test_repository_retries_once_after_clickhouse_network_error() -> None:
    class BrokenThenWorkingSource:
        def __init__(self) -> None:
            self.calls = 0
