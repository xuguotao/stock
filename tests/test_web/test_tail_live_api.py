...
def test_tail_live_selection_api_enriches_signal_credibility_with_historical_calibration(tmp_path) -> None:
    class FakeSignalRepository:
    ...
    def fake_runner(payload: TailLiveSelectionRequest, progress=None) -> dict[str, Any]:
        row = {
            ...
            "tail_return": 0.018,
            "v2_score": 88.0,
            "credibility": {"score": 87.0, "history": {"status": "样本不足", "sample_count": 0}},
        }
        return {
            "trade_date": payload.trade_date.isoformat(),
            "scanned_count": 1,
    ...
    assert response.status_code == 200
    ranked_history = job["result"]["ranked_signals"][0]["credibility"]["history"]
    selected_history = job["result"]["selections"][0]["credibility"]["history"]
    assert ranked_history["status"] == "ready"
...
def test_tail_live_selection_model_mode_reranks_final_candidates() -> None:
            ...
            return [
                {
                    ...
                    "expected_high_return": score / 100,
                    "risk_probability": 0.1,
                }
                for symbol, score in {"000001.SZ": 0.42, "000002.SZ": 0.86}.items()
            ]
    ...
    first = {
        ...
        "credibility": {"calibrated_probability": 0.8},
    ...
    second = {
        ...
        "credibility": {"calibrated_probability": 0.7},
        "tradability": {"buyable": True},
    }
    result = {
        "selected_count": 1,
    ...
    assert [row["symbol"] for row in result["ranked_signals"]] == ["000002.SZ", "000001.SZ"]
    assert [row["symbol"] for row in result["selections"]] == ["000002.SZ"]
    assert result["ranked_signals"][0]["status"] == "selected"
    assert result["ranked_signals"][1]["status"] == "filtered"
