...
- **overnight**：**折扣② 坐实（自相关）**。三个证据：(1) 因子定义=滞后隔夜→IC 本质是自相关；(2) decay 1→5 日总体衰减（0.359→0.145，非严格单调——h2=0.212→h3=0.232 微回升，但 h1 最强、h5 最弱），自相关特征；(3) turnover=0.978 极高（排名几乎天天变），可交易性差——虽有序列自相关，但"哪些票隔夜最强"每天大换血，无法固定持有。
...
> [!note] 口径说明
> 上表口径 A 的"top 分位"是 5 分位的最高档（≈全市场 top-20%，每档约 15 只/日），**不是 S01 的 top-5 持仓口径**。0.36%/日（年化~91%）是 top-20% 分位口径的量级，top-5 量级需接进回测引擎按真实持仓结算后才能确认。报告 §7 已标注此限制。
