...
**Architecture:** 每次 Mootdx 原始数据同步先获得一个不可复用的全局序号，并以同一序号写入日线和 XDXR 行；复权运行保存连续已结算边界，实际数据输入只消费其中 `succeeded` 的序号，构建范围为 `(previous_seq, captured_seq]`，并继续生成完整 raw/event/factor 快照。
...
def test_failed_sync_keeps_consumption_watermark_on_last_successful_sequence() -> None:
    ...
    assert latest_settled_ingest_seq(client) == 1
```
...
将 `research_adjustment_runs.input_watermark` 演进为 `input_ingest_seq Nullable(UInt64)`（保留旧字段仅用于历史兼容，不再作为增量边界）。`current_run()` 返回该序号；`publish_run()` 接受并持久化非空序号。构建开始时查询 `mootdx_ingestion_runs` 的连续已结算边界；目标标的分别从日线和 XDXR 取 `ingest_seq > previous_ingest_seq AND ingest_seq <= captured_ingest_seq` 且运行审计状态为 `succeeded` 的并集。所有原始读取限制到成功 `ingest_seq <= captured_ingest_seq`，并把实际输入写入现有 raw snapshot。
