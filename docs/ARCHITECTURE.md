...
- 数据获取与质量治理：以 ClickHouse 为主数据仓，维护股票列表、日线、分钟线、行情快照、快照聚合、基金尾盘数据和质量快照。
- 离线研究：从 ClickHouse 或 research parquet 数据集构建可复现实验样本，进行因子计算、IC 分析、分层分析和参数网格评估。
...
- `src.web.backend.fund_tail`：基金尾盘 Web 适配层，复用基金尾盘研究函数、ClickHouse 基金数据和基金监控池。
- `frontend/`：Vue 3 + TypeScript + Element Plus + ECharts 控制台，当前包含数据中心、任务中心、尾盘选择、策略复盘、基金尾盘和投资渠道页面。

Web 层只做编排、可视化、配置管理和一次性任务触发，不复制策略逻辑，也不应该承载必须长期运行的数据更新循环。数据中心主视图以“数据健康矩阵”和“更新任务状态”为核心：健康矩阵回答数据源现在是否可用，更新任务状态回答数据为什么会好/坏以及后续能否自动维持。

当前 Web 进程仍有若干进程内后台任务和守护状态，后续目标是拆为独立数据更新 runner：runner 脱离后台常驻运行，Web 只读取持久化任务状态、修改配置、手动触发一次性任务或暂停/恢复任务。

独立数据更新 runner 的目标边界：

- `src.data_ops.runner` 是数据任务执行面，可以部署到没有 Web 后台和前端的服务器。
- Web 后台是控制面，只读写 ClickHouse 中的任务配置、运行记录和心跳。
- `data_ops_task_config` 保存任务启停、调度参数和手动触发标记。
- `data_ops_task_runs` 保存每次运行结果和错误。
- `data_ops_task_heartbeats` 保存 runner 存活和当前任务心跳。
- runner 本地配置只保存 ClickHouse 连接、日志目录和 runner id，不保存业务任务状态。
- runner 支持按任务组拆分部署：`realtime` 负责快照采集和聚合刷新，`intraday` 负责 5m 分钟线同步，`maintenance` 负责质量快照和日终维护。交易时段不应让全市场 5m 同步阻塞实时快照链路。
...
- `src.data.clickhouse_source.ClickHouseStockDataSource`：ClickHouse 行情读取适配器，是后台、尾盘选择、数据中心和模型审计的主数据源。
- `src.data.clickhouse_minute5_sync`、`src.data.clickhouse_daily_sync`、`src.data.clickhouse_quote_snapshot_sync`：分钟线、日线修复和行情快照写入链路。

推荐边界：

- 联网数据获取和兜底仍可走 `DataAggregator`。
- 后台运行时、数据中心、尾盘选择和模型审计优先走 ClickHouse。
- 离线研究和可复现实验可以走 ClickHouse 或导出的 research parquet，但需要明确样本构建时间和数据口径。
